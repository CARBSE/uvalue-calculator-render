...
// We no longer need jsPDF or html2canvas
...
import { getMaterials, getCities, getFilms, postCalculate } from "./lib/api";

const INITIAL_ASSEMBLY = "wall";

const getInitialState = () => {
  // This function now always returns a blank slate.
  return {
    layers: [],
    city: "",
...
export default function App() {
  ...
  const matsByName = useMemo(() => {
    const map = new Map(); materials.forEach(m => map.set(m.name, m)); return map;
  }, [materials]);
  ...
  // Simplified handleSave function that calls the browser's print dialog
  const handleSave = () => {
    window.print();
  };
      ...
      <div id="capture-area" className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-9">
          <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-gray-100 rounded-lg mb-4">
            ...
            <button className="px-4 py-1 bg-white border rounded shadow-sm hover:bg-gray-50" onClick={addLayer}>Add Layer</button>
          </div>
